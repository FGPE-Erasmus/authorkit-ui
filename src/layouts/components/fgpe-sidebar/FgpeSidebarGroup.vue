  ...
  >
    <div @click="clickGroup" class="group-header w-full">
      <span class="flex items-center w-full">
        ...
        />
        <span v-show="!sidebarItemsIconOnly" class="truncate mr-3">{{
          $t(group.i18n) || group.name
        }}</span>
        <vs-chip
          class="ml-auto mr-4"
          :color="group.tagColor"
          v-if="group.tag && !sidebarItemsIconOnly"
          >{{ group.tag }}</vs-chip
        >
      </span>
      <feather-icon
...
export default {
  ...
  watch: {
    ...
    $route() {
      if (this.sidebarItemsIconOnly) return;
    ...
    "$store.state.sidebarItemsIconOnly"(val) {
