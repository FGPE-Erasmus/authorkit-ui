...
export const suggestions = [
  ...
  {
    description:
      "select an item by it's index relatively to the size of the collection",
    value: "(@.length-1)",
    setCarretAt: 0,
    scopes: ["[]"]
  },
...
export const getSuggestions = (
  ...
  try {
    ...
    const filteredJson = /*jp.query(jsonToTestAgainst, jsonPath)*/ JSONPath({
      json: jsonToTestAgainst,
      path: jsonPath,
      sandbox
    })[0];
    if (Array.isArray(filteredJson)) {
      ...
      const parentJsonPath = jsonPath.substring(
        0,
        jsonPath.lastIndexOf(".") + 1
      );
      // Compute extra json path value, to be used to filter available suggestions
      const filterSuggestionJsonPath = jsonPath.substring(
        jsonPath.lastIndexOf(".") + 1
      );
      let additionalSuggestions = [];
      if (parentJsonPath) {
        const applicableScopes = suggestions.filter(s =>
          parentJsonPath.endsWith(s.value)
        );
        additionalSuggestions = guessSuggestionsFromScopes(
          applicableScopes,
          caretPosition,
          parentJsonPath,
          jsonToTestAgainst
      ...
      return [
        ...additionalSuggestions.filter(
          s =>
            s.value.includes(filterSuggestionJsonPath) &&
            s.value !== filterSuggestionJsonPath
        ),
        ...suggestions.filter(s => s.scopes.includes("object"))
      ];
    }
  } catch (e) {
    const applicableScopes = suggestions.filter(s => {
      if (typeof s.setCarretAt !== undefined) {
        ...
        return (
          jsonPathPartBeforeCarret.endsWith(valueToBeBeforeCarret) &&
          jsonPathPartAfterCarret.startsWith(valueToBeAfterCarret)
        );
      }

      return jsonPath.endsWith(s.value);
    });
    return guessSuggestionsFromScopes(
      applicableScopes,
      caretPosition,
      jsonPath,
      jsonToTestAgainst,
...
const guessSuggestionsFromScopes = (
  scopes,
  carretPosition,
  jsonPath,
  jsonToTestAgainst,
  ...
  return flatten(
    scopes.map(applicableScope => {
      // Check if there is any conditions on carret position for applicableScope
      if (applicableScope.setCarretAt) {
        if (jsonPath.length - carretPosition === applicableScope.setCarretAt) {
          return (
            flatten(
              evalAllProperties(
                suggestions.filter(s =>
                  s.scopes.includes(applicableScope.value)
                ),
                carretPosition,
                jsonPath,
                jsonToTestAgainst,
        ...
        return flatten(
          evalAllProperties(
            suggestions.filter(s => s.scopes.includes(applicableScope.value)),
            carretPosition,
            jsonPath,
            jsonToTestAgainst,
            sandbox
...
export const evalAllProperties = (
  suggestions,
  carretPosition,
  jsonPath,
  jsonToTestAgainst,
  ...
  return suggestions.map(s => {
    if (s.value === "all_properties") {
      ...
      try {
        const filteredJson = /*jp.query(jsonToTestAgainst, jsonPathToObject)*/ JSONPath(
          {
            json: jsonToTestAgainst,
            path: jsonPathToObject,
            sandbox
          }
        )[0];
      ...
      try {
        ...
        const properties = Array.from(
          new Set(getAllPropertiesRecursively(filteredJson))
        );
        return properties.map(p => ({
          value: p,
          description: "property",
      ...
      //"$.d[1].df.f"
      const lastIndexOfOpenedArray = splittedJsonPathAtCursor.lastIndexOf("[");
      const jsonPathToParentArray = splittedJsonPathAtCursor.substring(
        0,
        lastIndexOfOpenedArray
      );
      // 3. We then want to evaluate all unique properties in f array at first level
      try {
        const filteredJson = /*jp.query(jsonToTestAgainst, jsonPathToParentArray)*/ JSONPath(
          {
            json: jsonToTestAgainst,
            path: jsonPathToParentArray,
            sandbox
          }
        )[0];
        const properties = Array.from(
          new Set(getAllPropertiesAtFirstLevel(filteredJson))
        );
        return properties.map(p => ({
          value: p + " ",
...
const getAllPropertiesRecursively = objectOrArray => {
  if (Array.isArray(objectOrArray)) {
    ...
    const subkeys = keys
      ...
      .filter(k => k !== null);
    return flatten([...keys, ...subkeys]).filter(
