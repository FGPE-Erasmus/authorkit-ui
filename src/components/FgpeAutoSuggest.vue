  ...
  <div class="fgpe-auto-suggest">
    ...
    >
      <li
        ...
        :key="index"
        :class="{
          'fgpe-auto-suggest__current-selected': currentSelected == index,
...
<script>
import { THEME_TOGGLE_CONTENT_OVERLAY } from "@/store/constants";
    ...
    searchQuery(val) {
      if (val == "") {
        ...
        if (this.backgroundOverlay && !this.bodyOverlay)
          this.$store.commit(THEME_TOGGLE_CONTENT_OVERLAY, true);
  ...
  computed: {
    ...
    actionClasses() {
      return isHighlighted => {
        if (isHighlighted) return `stroke-current text-warning`;
      };
    }
  },
  methods: {
    ...
    inputInit() {
      if (this.showPinned) {
        const starredData = this.data.filter(item => item.highlightAction);
